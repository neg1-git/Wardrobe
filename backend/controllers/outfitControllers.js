...
try {
  ...
  const result= await db.query(`select o.id as outfit_id, o.name, o.wear_count, o.last_worn_date, o.is_favorite,
    json_agg(json_build_object('id', c.id, 'name', c.name, 'category', c.category, 'color', c.color, 'image_url', c.image_url)) as clothes
    from outfit_items oi
    JOIN clothing_items c ON oi.clothing_item_id = c.id
    JOIN outfits o ON oi.outfit_id = o.id
    where o.user_id=$1
    group by o.id, o.name, o.wear_count, o.last_worn_date, o.is_favorite`,[id])
